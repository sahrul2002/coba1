...
export const dataService = {
    ...
    loadAllData: async function() {
        try {
            ...
            const [kerusakan, gejala, rules] = await Promise.all([
                databaseService.getAllKerusakan(),
                databaseService.getAllGejala(),
    ...
    setupRealtimeListeners: function() {
        ...
        databaseService.addEventListener('kerusakan', (payload) => {
            this.handleRealtimeChange('kerusakan', payload);
        });
        
        // Listen for gejala changes
        databaseService.addEventListener('gejala', (payload) => {
            this.handleRealtimeChange('gejala', payload);
        });
        
        // Listen for rules changes
        databaseService.addEventListener('rules', (payload) => {
    ...
    addKerusakan: async function(kerusakan) {
        if (this.usingDatabase) {
            const result = await databaseService.addKerusakan(kerusakan);
            if (result.success) {
    ...
    updateKerusakan: async function(kode, updatedKerusakan) {
        if (this.usingDatabase) {
            const result = await databaseService.updateKerusakan(kode, updatedKerusakan);
            if (result.success) {
    ...
    deleteKerusakan: async function(kode) {
        if (this.usingDatabase) {
            const result = await databaseService.deleteKerusakan(kode);
            if (result.success) {
    ...
    addGejala: async function(gejala) {
        if (this.usingDatabase) {
            const result = await databaseService.addGejala(gejala);
            if (result.success) {
    ...
    updateGejala: async function(kode, updatedGejala) {
        if (this.usingDatabase) {
            const result = await databaseService.updateGejala(kode, updatedGejala);
            if (result.success) {
    ...
    deleteGejala: async function(kode) {
        if (this.usingDatabase) {
            const result = await databaseService.deleteGejala(kode);
            if (result.success) {
    ...
    addOrUpdateRule: async function(rule) {
        if (this.usingDatabase) {
            const result = await databaseService.addOrUpdateRule(rule);
            if (result.success) {
    ...
    deleteRule: async function(kode_kerusakan, kode_gejala) {
        if (this.usingDatabase) {
            const result = await databaseService.deleteRule(kode_kerusakan, kode_gejala);
            if (result.success) {
